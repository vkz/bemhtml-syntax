...
addBeautfierOpts(cmd)
  .opt()
    .name('format').title('output "bemhtml" or "bh" or "krasota" (default: "bemhtml")')
    .short('f').long('format')
    .val(function (v) {
      if (v === 'bemhtml' || v === 'bh' || v === 'krasota')
        return v;
      return this.reject('output format must be "bemhtml" or "bh" or "krasota"');
    })
    .req()
    .def('bemhtml')
    .end()
