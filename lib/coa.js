...
cmd
  ...
  .opt()
    ...
    .end()
  .opt()
    .name('applySetsMode').title('Assert that every call to apply() sets a mode in its 1st argument. (default: false)')
    .short('aM').long('apply-sets-mode')
    .flag()
    .end()
  .opt()
    .name('applyCheckFields').title('Warn if apply() sets any of "block", "elem", "mods" or "elemMods". (default: false)')
    .short('aC').long('apply-check-fields')
    .flag()
    .end()

addBeautfierOpts(cmd)
  .opt()
    .name('format').title('template format "bh" or "bemxjst1" or "bemxjst4" (default: "bemxjst4")')
    .short('f').long('format')
    .val(function (v) {
      if (v === 'bemhtml' || v === 'bh' ||
          v === 'krasota' || v === 'bemxjst1' ||
          v === 'bemxjst4')
        return v;
      return this.reject('format must be "bh" or "bemxjst1" or "bemxjst4"');
    })
    .req()
    .def('krasota')
    .end()
